...
// 1. Create a new `n`-element (where `n` is the length of `arr`) array named `rotated` to hold the 
// rotated items.
// 2. Copy the contents of `arr` over to the new array in two parts:
// The `d`-element contiguous segment from `arr_0` to `arr_d-1` must be copied over to the contiguous 
// segment starting at `rotated_n-d` and ending at `rotated_n-1`.
// The `(n-d)`-element contiguous segment from `arr_d` to `arr_n-1` must be copied over to the contiguous 
// segment starting at `rotated_0` and ending at `rotated_d`.
// Reassign the reference to `arr` so that it points to `rotated` instead.
